...
// Testing functions.
const InstVector &NDRange::getGlobalIds(int direction) const {
  assert(direction >= 0 && direction < static_cast<int>(oclInsts.size()) &&
         "direction input is not within range");
  const auto &directionMap = oclInsts[direction];
  return directionMap.at(GET_GLOBAL_ID);
}

const InstVector &NDRange::getLocalIds(int direction) const {
  assert(direction >= 0 && direction < static_cast<int>(oclInsts.size()) &&
         "direction input is not within range");
  const auto &directionMap = oclInsts[direction];
  return directionMap.at(GET_LOCAL_ID);
}
...
// -----------------------------------------------------------------------------
void SymEngine::findOpenCLFunctionCallsByName(std::string calleeName, Function *caller,
...
// -----------------------------------------------------------------------------
void SymEngine::findOpenCLFunctionCalls(Function *callee, Function *caller,
                             int direction, InstVector &target) {
